...
db = pd.merge(left=db, right=rws, how="left", left_on="sample", right_on="bottleid")
...
rn = {"temperature": "temperature_is"}
db.rename(mapper=rn, axis=1, inplace=True)

# process airica data
db = process_airica(2012.59, db, "./data/LDRWS1.dbs", "./results_rws.csv")
