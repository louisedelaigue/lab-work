...
def process_ph_keep20(file_path, results_file_path_and_name):
    ...
    # import spreadsheet
    db = pd.read_excel(file_path, skiprows=[1])
    ...
    for file in file_list:
        ...
        S = data[file].sec < 1200.050
        data[file] = data[file][S]
        ...
        # store rest of info
        results.loc[results.filename == file, "analysis_date"] = db.analysis_date
        results.loc[results.filename == file, "location"] = db.location
        results.loc[results.filename == file, "T_room"] = db.T_room
        results.loc[results.filename == file, "T_bath"] = db.T_bath
        results.loc[results.filename == file, "dPhi_4"] = db.dPhi_4
        results.loc[results.filename == file, "dPhi_10"] = db.dPhi_10
        results.loc[results.filename == file, "offset"] = db.offset
        results.loc[results.filename == file, "pH_CRM_calc"] = db.pH_CRM_calc
        results.loc[results.filename == file, "pH_opt"] = db.pH_opt
        results.loc[results.filename == file, "time_to_eq"] = db.time_to_eq
        results.loc[results.filename == file, "flag"] = db.flag
